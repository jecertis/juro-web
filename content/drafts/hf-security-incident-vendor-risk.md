...
3. **RESOLVED 2026-07-20 — Naming Hugging Face directly in title/H1/meta.** Founder confirmed: proceed as recommended. Hugging Face is now named directly in title, H1, meta description, OG/Twitter tags, and Article JSON-LD `headline`/`description` in the built page (`blog/hf-security-incident-vendor-risk-2026.html`).
...
5. **RESOLVED 2026-07-20 — Article schema `author` field.** Founder confirmed: `article:author` = "JuroCompliant", matching existing site-wide convention; no workspace-wide change needed. Built page uses `"JuroCompliant"` for `article:author`, `<meta name="author">`, visible byline, and JSON-LD `author`/`publisher` (with `@id` linking to the canonical `https://jurocompliant.com/#organization` node defined in `index.html`, per the same inline-object-plus-`@id` pattern used elsewhere in the `@graph` — note this is the first blog post to attach the `@id`; all 8 prior posts use a bare inline Organization object with no `@id`, so this is a small enhancement flagged for founder awareness, not a silent site-wide convention change).
